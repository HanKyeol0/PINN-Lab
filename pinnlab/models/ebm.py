...
class EBM():
    ...
    def __init__(self, pars, ymax, device):
        self.ebm_ubound = pars['ebm_ubound'] # upper bound for EBM residuals
        self.Uvec_ebm = pars['Uvec_ebm']
        self.fdrop_ebm = pars['fdrop_ebm'] # dropout rate for EBM
        self.lr_ebm = pars['lr_ebm']
        self.bs_train = pars['bs_train'] # batch size for EBM training
        self.Nebm = pars['Nebm'] # number of EBM training iterations
        ...
        self.net_ebm = Net(self.Uvec_ebm, fdrop=self.fdrop_ebm, bounds = self.ebm_bounds).to(device) # EBM network
